...
class AdvancedStripeChecker:
    ...
    async def create_token(self, session: httpx.AsyncClient, card: CardDetails) -> Dict:
        ...
        try:
            ...
            # Authentication should be done via Bearer token in headers
            # Use PK if available, else SK
            auth_token = self.account.pk if (hasattr(self.account, 'pk') and self.account.pk) else self.account.sk
    ...
    async def test_single_card(self, card: CardDetails, proxy_url: Optional[str] = None, card_str: str = "") -> Dict:
        ...
        try:
            # IMPORTANT: Store current account reference before creating new session
            account = self.account
            
            # Create session with proxy
            session = await self.create_session(proxy_url)
            try:
                # Restore account reference for the new session scope
                self.account = account
                
                if not self.account or not self.account.sk:
                    result["message"] = "Account information not fetched. Please validate SK first."
                    return result

                # Create browser session for this card
                self.create_browser_session()
                
                # Extract PK if not present
                if not self.account.pk:
                    await self.extract_pk(session)
                ...
                if test_result.get("success"):
                    ...
                    if status == "succeeded":
                        ...
                        result["message"] = "Decline (requires payment method)"
